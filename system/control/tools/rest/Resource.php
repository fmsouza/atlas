...
abstract class Resource implements Responsibility{
    ...
    public function processRequest($request){
        if(!$this->request)
            throw new InvalidArgumentException(Main::$strings->getKey("undefined_service"));
        elseif(!self::$method)
            throw new InvalidArgumentException(Main::$strings->getKey("method_not_set"));
    ...
    protected function head(){
        ...
        $request = new HttpRequest(Config::base_url());
    ...
    protected function success($msg=null){
        if(is_null($msg)) $msg = Main::$strings->getKey("operation_success");
        return $this->message(Main::$strings->getKey("success"),$msg);
    }
    
    protected function error($msg=null){
        if(is_null($msg)) $msg = Main::$strings->getKey("internal_error");
        return $this->message(Main::$strings->getKey("error"),$msg);
    }
    
    protected function message($type,$msg){
        $json = new JsonObject();
        $json->setKey(Main::$strings->getKey("type"),$type);
        $json->setKey(Main::$strings->getKey("message"),$msg);
        return $json;
    }
}